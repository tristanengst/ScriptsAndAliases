...
def get_slurm_status(cur_user=False, account=None, verbose=False):
    ...
    keys = ["JOBID", "USER", "STATE", "START_TIME", "TIME_LEFT", "Gres", "NODES", "NAME", "REASON", "ACCOUNT", "PARTITION", "HOST", "EXC_NODES", "COMMENT"]
    
    
    squeue_cmd = f"squeue {user_str} {account_str} -h -o \"%i|%u|%T|%S|%L|%b|%D|%j|%r|%a|%P|%N|%x|%k\""
    ...
    job2comment = {j[0]: " ".join(j[13:]) for j in jobs}
    ...
    squeue_cmd = f"squeue {user_str} {account_str} -h -O \"JobID:10,SubmitTime:20,EligibleTime:20\""
    squeue = subprocess.getoutput(squeue_cmd).strip()
    if verbose:
        print(f"Running command: {squeue_cmd}")
        print(f"Output:\n{squeue}")
